...
import { ClipboardCheck, Filter, ListFilter, X, ChevronDown, Check } from 'lucide-react';
...
export function CategoryGradeCard({ data, className }: CategoryGradeCardProps) {
    ...
    }, [data.subjects, selectedSemester, selectedCategory]);
    ...
    const isFiltered = selectedSemester !== 'All Semesters' || selectedCategory !== 'All Categories';
        ...
        >
            <div className="flex items-center justify-between border-b border-zinc-100 p-5 dark:border-zinc-900/50">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h3 className="text-lg font-black text-zinc-900 dark:text-zinc-50 tracking-tight">Category Grades</h3>
                        <p className="text-[11px] font-bold text-zinc-400 uppercase tracking-wider">Academic Record</p>
                    </div>
                </div>
            </div>
            ...
            <div className="flex flex-col gap-5 p-6 bg-zinc-50/40 dark:bg-zinc-900/20 border-b border-zinc-100 dark:border-zinc-900">
                <div className="flex flex-wrap items-center gap-4">
                    <div className="flex items-center gap-2 pr-4 border-r border-zinc-200 dark:border-zinc-800">
                        ...
                        <span className="text-[10px] font-black uppercase text-zinc-400 tracking-widest">Filter by</span>
                    </div>
                    ...
                    <div className="flex flex-wrap gap-2.5 items-center">
                        ...
                        <div className="group relative">
                            <select
                                ...
                                className={cn(
                                    "appearance-none rounded-full border px-4 py-1.5 text-[11px] font-black outline-none transition-all cursor-pointer pr-8 pl-4 shadow-sm",
                            ...
                            >
                                {semesters.map(sem => (
                                    <option key={sem} value={sem} className="text-zinc-900 dark:text-zinc-100 font-medium">{sem}</option>
                                ))}
                            </select>
                        ...
                        <div className="group relative">
                            ...
                            >
                                {categories.map(cat => (
                                    <option key={cat} value={cat} className="text-zinc-900 dark:text-zinc-100 font-medium">{cat}</option>
                                ))}
                            </select>
                            <ChevronDown className={cn(
                                "absolute right-2.5 top-1/2 -translate-y-1/2 h-3 w-3 pointer-events-none transition-transform group-hover:translate-y-[-40%]",
                        ...
                        {isFiltered && (
                            <button 
                                onClick={() => { setSelectedSemester('All Semesters'); setSelectedCategory('All Categories'); }}
                ...
                <div className="flex items-center justify-between px-1">
                    <div className="flex items-center gap-6">
                        <div className="flex items-center gap-2">
                            ...
                            <span className="text-[10px] font-bold text-zinc-400 uppercase tracking-tight">Courses</span>
                            <span className="text-xs font-black text-zinc-900 dark:text-zinc-100 tabular-nums">{stats.count}</span>
                        </div>
                        <div className="flex items-center gap-2">
                            <div className="h-1.5 w-1.5 rounded-full bg-emerald-500" />
                            <span className="text-[10px] font-bold text-zinc-400 uppercase tracking-tight">Total Credits</span>
                            <span className="text-xs font-black text-zinc-900 dark:text-zinc-100 tabular-nums">{stats.totalCredits}</span>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <table className="w-full text-left text-sm">
                    <thead>
                        <tr className="bg-zinc-50/50 dark:bg-zinc-900/30 border-b border-zinc-100 dark:border-zinc-900 text-[10px] font-black uppercase text-zinc-400 tracking-widest">
                            ...
                            <th className="py-4 px-2 text-center">CR</th>
                            <th className="py-4 pr-6 pl-2 text-right">Grade</th>
                        </tr>
                    </thead>
                    <tbody className="divide-y divide-zinc-50 dark:divide-zinc-900/50">
                        {filteredSubjects.map((subject, idx) => {
                            const isPassing = subject.grade === 'P' || (subject.grade && !['F', 'U'].includes(subject.grade));
                            const isHighGrade = ['A+', 'A0', 'A-'].includes(subject.grade);
                            const hasCategory = !!subject.category;

                            return (
                                <tr key={`${subject.code}-${idx}`} className="group transition-all hover:bg-zinc-50/80 dark:hover:bg-zinc-900/40">
                                    <td className="py-4 pl-6 pr-2">
                                        <div className="flex flex-col gap-0.5">
                                            <span className="font-bold text-zinc-900 dark:text-zinc-50 text-xs line-clamp-1 group-hover:text-primary transition-colors">
                                                {subject.name}
                                            </span>
                                            <div className="flex items-center gap-2">
                                                <span className="text-[9px] font-black text-zinc-400 tabular-nums bg-zinc-100 dark:bg-zinc-800 px-1 rounded">
                                                    {subject.code}
                                                </span>
                                                {subject.info && (
                                    ...
                                    <td className="py-4 px-2 text-center">
                                        <span className="text-[10px] font-black text-zinc-500 dark:text-zinc-400 tabular-nums">
                                            {subject.yearSemester.year}-{subject.yearSemester.semester}
                                        </span>
                                    </td>
                                    <td className="py-4 px-2 text-center">
                                        <span className={cn(
                                            "inline-flex rounded-md px-2 py-0.5 text-[9px] font-black whitespace-nowrap tracking-tight",
                                            hasCategory 
                                    ...
                                    <td className="py-4 px-2 text-center font-bold text-xs text-zinc-600 dark:text-zinc-400 tabular-nums">
                                        {subject.credit}
                                    </td>
                                    <td className="py-4 pr-6 pl-2 text-right">
                                        <span
                                            className={cn(
                                                'inline-flex min-w-[2.2rem] justify-center rounded-lg px-2 py-1 text-xs font-black tabular-nums shadow-sm border',
                                                isHighGrade 
