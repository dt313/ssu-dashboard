                        ...
                        <tbody className="divide-y divide-zinc-100 dark:divide-zinc-900">
                            {[...data].reverse().map((item, idx) => (
                                ...
                                >
                                    ...
                                    <td className="py-4 px-2 font-bold text-zinc-900 dark:text-zinc-50 text-xs max-w-[200px] truncate">
                                        {item.name}
                                    </td>
                                    <td className="py-4 px-2 text-zinc-500 dark:text-zinc-400 font-bold text-center tabular-nums text-xs">
                                        {item.type}
                                    </td>
                                    <td className="py-4 px-2 text-right font-black text-zinc-900 dark:text-zinc-50 tabular-nums text-xs">
                                        {item.amount}
                                    </td>
                                    <td className="py-4 pl-2 text-right relative">
                                        <span
                                            className={cn(
                                                'inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-[10px]font-bold cursor-pointer',
                                        ...
                                        {isRejected(item.status) && tooltipIndex === idx && item.rejectReason && (
                                            <div className="absolute right-0 top-full z-50 mt-2 w-48 rounded-lg bg-zinc-900 p-3 text-xs text-white shadow-lg dark:bg-zinc-800">
                                                <p className="font-bold mb-1">탈락사유</p>
                                                <p className="text-zinc-300">{item.rejectReason}</p>
                                            </div>
                                        )}
                                    </td>
                                </tr>
